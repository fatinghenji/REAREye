...
namespace {
    ...
    constexpr const char *kEnvOriginalBinary = "REAREYE_GALLERY_ORIGINAL_BINARY";
    ...
    constexpr int kFlutterAppPollAttempts = 80;
    constexpr useconds_t kFlutterAppPollIntervalUs = 50000;
    ...
    std::atomic_bool flutter_app_poll_started{false};
    ...
    [[maybe_unused]] bool ends_with(const char *text, const char *suffix) {
    ...
    [[maybe_unused]] bool
    matches_formatter_ctor_candidate(const ExecutableMapping &mapping, uintptr_t address) {
        if (!contains_range(mapping, address, 0x60) ||
            !looks_like_flutter_aot_function(mapping, address))
            return false;
        // Privacy watermark's input formatter constructor stores a Dart Smi maxLength (14 << 1)
        // into the freshly allocated object, then passes the raw maxLength 14 as a stack argument.
        // Keep the match structural so minor AOT pool offset changes do not break it.
        return is_bl(instruction_at(address, 3)) &&
               is_mov_reg(instruction_at(address, 4), 4, 0) &&
               is_add_imm_to_reg(instruction_at(address, 5), 0, 27) &&
               is_ldr_x_unsigned_from(instruction_at(address, 6), 0, 0) &&
               instruction_at(address, 7) == 0xf81f83a4u &&
               is_stur_w_to(instruction_at(address, 8), 4, 0x2B) &&
               is_movz_x_imm(instruction_at(address, 9), 0, 0x1C) &&
               is_stur_x_to(instruction_at(address, 10), 4, 0x2F) &&
               is_movz_x_imm(instruction_at(address, 11), 16, 0xE) &&
               is_str_x_unsigned_to(instruction_at(address, 12), 15, 0) &&
               is_mov_reg(instruction_at(address, 13), 1, 4) &&
               is_add_imm_to_reg(instruction_at(address, 14), 2, 27) &&
               is_ldr_x_unsigned_from(instruction_at(address, 15), 2, 2) &&
               is_movz_x_imm(instruction_at(address, 16), 3, 8) &&
               is_movz_x_imm(instruction_at(address, 17), 5, 0x40) &&
               is_ldr_x_unsigned_from(instruction_at(address, 18), 4, 27) &&
               is_bl(instruction_at(address, 19));
    }
    ...
    bool matches_setter_text_limit_cmp(const ExecutableMapping &mapping, uintptr_t cmp) {
        if (cmp < mapping.start + 11 * sizeof(uint32_t)) return false;
        const uintptr_t setter = cmp - 14 * sizeof(uint32_t);
        if (!contains_range(mapping, setter, 24 * sizeof(uint32_t)) ||
            !looks_like_flutter_aot_function(mapping, setter))
            return false;
        return instruction_at(setter, 3) == 0xaa0103e0u &&
               is_ldur_w_from(instruction_at(setter, 4), 0, 0x27) &&
               instruction_at(setter, 5) == 0x8b1c0021u &&
               instruction_at(setter, 10) == 0x36000041u &&
               instruction_at(setter, 11) == 0xf8407023u &&
               is_cmp_x_imm(instruction_at(cmp), 0xE) &&
               instruction_at(setter, 15) == 0x5400006du &&
               instruction_at(setter, 16) == 0xaa0003e1u &&
    ...
    bool matches_controller_text_limit_cmp(const ExecutableMapping &mapping, uintptr_t cmp) {
        if (cmp < mapping.start + 8 * sizeof(uint32_t)) return false;
        const uintptr_t fn = cmp - 10 * sizeof(uint32_t);
        if (!contains_range(mapping, fn, 18 * sizeof(uint32_t)) ||
            !looks_like_flutter_aot_function(mapping, fn))
            return false;
        return instruction_at(fn, 3) == 0xaa0203e0u &&
               instruction_at(fn, 4) == 0xf81e83a2u &&
               instruction_at(fn, 5) == 0xaa0303e2u &&
               instruction_at(fn, 6) == 0xf81e03a3u &&
               instruction_at(fn, 7) == 0xaa0103e3u &&
               is_ldur_x_from(instruction_at(fn, 9), 3, 0x37) &&
               is_cmp_x_imm(instruction_at(cmp), 0xE) &&
               instruction_at(fn, 11) == 0x5400006du &&
               instruction_at(fn, 12) == 0xaa0303e1u &&
    ...
    void install_flutter_app_hooks(void *handle, const char *source) {
        ...
        if (!find_executable_mapping(kFlutterAppLibraryName, &mapping)) {
            reareye::nativehook::log_warn(
                    "flutter app load observed but executable mapping missing source=%s handle=%p privacy_watermark=%d",
        ...
        reareye::nativehook::log_info(
                "flutter app load observed source=%s handle=%p base=%p text=%p-%p privacy_watermark=%d",
                source != nullptr ? source : "<unknown>",
                handle,
                reinterpret_cast<void *>(mapping.base),
                reinterpret_cast<void *>(mapping.start),
    ...
    void *flutter_app_poll_worker(void *) {
        reareye::nativehook::log_info(
                "flutter app poll start attempts=%d interval_us=%u",
                kFlutterAppPollAttempts,
                static_cast<unsigned>(kFlutterAppPollIntervalUs)
        );
        for (int attempt = 1; attempt <= kFlutterAppPollAttempts; ++attempt) {
            if (flutter_app_hooks_installed.load(std::memory_order_acquire)) break;
            void *handle = dlopen(kFlutterAppLibraryName, RTLD_NOW | RTLD_NOLOAD);
            ExecutableMapping mapping{};
            if (find_executable_mapping(kFlutterAppLibraryName, &mapping)) {
                reareye::nativehook::log_info(
                        "flutter app poll detected attempt=%d handle=%p base=%p text=%p-%p",
                        attempt,
                        handle,
                        reinterpret_cast<void *>(mapping.base),
                        reinterpret_cast<void *>(mapping.start),
                ...
                install_flutter_app_hooks(handle, "native-poll");
                if (flutter_app_hooks_installed.load(std::memory_order_acquire)) break;
            } else if (handle != nullptr) {
                reareye::nativehook::log_info(
                        "flutter app poll handle exists but mapping missing attempt=%d handle=%p",
                        attempt, handle);
            }
            usleep(kFlutterAppPollIntervalUs);
        }
        reareye::nativehook::log_info(
                "flutter app poll finish installed=%d",
                flutter_app_hooks_installed.load(std::memory_order_acquire) ? 1 : 0
        );
        return nullptr;
    }

    void start_flutter_app_poll_if_needed() {
        if (!config.enable_privacy_watermark) return;
        bool expected = false;
        if (!flutter_app_poll_started.compare_exchange_strong(expected, true,
                                                              std::memory_order_acq_rel))
            return;
        ...
        if (pthread_attr_init(&attr) == 0) {
            ...
            const int status = pthread_create(&thread, &attr, flutter_app_poll_worker, nullptr);
            pthread_attr_destroy(&attr);
            reareye::nativehook::log_info("flutter app poll thread start result=%d", status);
            if (status != 0) {
                flutter_app_poll_started.store(false, std::memory_order_release);
            }
            return;
        }

        const int status = pthread_create(&thread, nullptr, flutter_app_poll_worker, nullptr);
        reareye::nativehook::log_info("flutter app poll thread start result=%d", status);
        if (status != 0) {
            flutter_app_poll_started.store(false, std::memory_order_release);
            return;
    ...
    void install_gallery_hooks() {
        ...
        const std::string original = read_runtime_value(kEnvOriginalBinary, "");
        reareye::nativehook::log_info("install gallery hooks config original=%s",
                                      original.empty() ? "<empty>" : original.c_str());
        load_config();
        ...
        start_flutter_app_poll_if_needed();
    }

    std::string resolve_original_library_path() {
        std::string value = read_runtime_value(kEnvOriginalBinary, "");
        if (!value.empty()) {
            reareye::nativehook::log_info("resolve original source=module-env path=%s",
                                          value.c_str());
            return value;
        }
    ...
    void prepare_original_entry() {
        std::call_once(init_once, [] {
            ...
            reareye::nativehook::log_info("gallery wrapper ready path=%s", original_path.c_str());
        });
    }
...
void *reareye_gallery_prepare_entry() {
    reareye::nativehook::set_log_tag(kLogTag);
...
void app_entry_point() {
    asm volatile(
            ...
            "stp x6, x7, [sp, #64]\n"
            "str x8, [sp, #80]\n"
            "bl reareye_gallery_prepare_entry\n"
            "mov x9, x0\n"
            "ldr x8, [sp, #80]\n"
            "ldp x6, x7, [sp, #64]\n"
            "ldp x4, x5, [sp, #48]\n"
